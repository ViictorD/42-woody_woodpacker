...
static void	manage_elf(void *ptr, size_t ori_size)
{
	...
	// my_code_offset += (load_segment->p_memsz - load_segment->p_filesz);
	// printf("4\n");

	// 5. reindex les sections
	reindex_sections(woody_ptr, load_segment, woody_size, my_code_offset, ori_size);
	// printf("5\n");
	
	// 6. Rajoute une section
	// append_new_section(woody_ptr, load_segment, my_code_offset, woody_size, bss_sec);
	// printf("6\n");
	...
	// 7. on pointe le debut du program sur la section
...
// ---------- TOOD ------------
// - Le projet marche que pour les binaire exectuable pas pour les "shared object" ou quoi
// - faire l'opt pour la key parametrable
// - mettre au propre